...
numTests =
  testGroup
    ...
    [ -- testCase "abs 1" $
      ...
      testCase "(+) 1" $
        matrix1 + matrix2 @?= (M [[2, 0], [8, 0]]),
      -- testCase "(+) 2" $
      --   matrix_3x0 + matrix2 @?= fehlerwert,
      -- testCase "(+) 3" $
      --   matrix1 + matrix_ungueltig @?= fehlerwert,
      testCase "(-) 1" $
        matrix1 - matrix1 @?= (M [[0, 0], [0, 0]]),
      -- testCase "(-) 2" $
      --   matrix_3x0 + matrix2 @?= fehlerwert,
      -- testCase "(-) 3" $
      --   matrix1 + matrix_ungueltig @?= fehlerwert,
      testCase "(*) 1" $
        (M [[1, 2, 3], [4, 5, 6]]) * (M [[1, 2], [1, 2], [1, 2]]) @?= (M [[6, 12], [15, 30]]),
      -- testCase "(*) 2" $
      --   matrix_3x0 * matrix2 @?= fehlerwert,
      -- testCase "(*) 3" $
      --   matrix1 + matrix_ungueltig @?= fehlerwert,
      testCase "negate 1" $
        negate (M [[1, 2, 3], [4, 5, 6]]) @?= (M [[-1, -2, -3], [-4, -5, -6]]),
      testCase "negate 2" $
        negate matrix1 @?= (M [[-1, -2], [-3, -4]]),
      testCase "signum 1" $
        signum matrix1 @?= (M [[1]]),
      testCase "signum 2" $
        signum (M [[-1, -2, -3], [-4, -5, -6]]) @?= (M [[-1]]),
      testCase "signum 3" $
        signum (M [[-1, -2, 3], [-4, 5, -6]]) `expectError` "Vorzeichenfunktion undefiniert",
      testCase "signum 3" $
        signum (M [[0, 0], [0, 0]]) @?= (M [[0]]),
      testCase "fromInteger 1" $
        fromInteger 1 @?= (M [[1]]),
      testCase "fromInteger 3" $
        fromInteger (-1) @?= (M [[-1]]),
      testCase "fromInteger 3" $
        fromInteger 0 @?= (M [[0]])
    ]
...
utilsTests =
  testGroup
    ...
    [ testCase "compareRows 1" $
      ...
      testCase "sumRows 2" $
        sumRows [-1, 2] [1, -2] @?= [0, 0],
      testCase "sumMatrix 1" $
        sumMatrix matrix1 matrix2 @?= (M [[2, 0], [8, 0]])
    ]
